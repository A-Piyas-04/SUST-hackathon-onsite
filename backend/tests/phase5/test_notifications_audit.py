...
def test_mutation_writes_audit_event_atomically(client, bkash_ops_headers):
...
def test_failed_mutation_writes_no_audit_event(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
    before = client.get(f"/api/v1/cases/{cid}/audit-events", headers=bkash_ops_headers).json()
...
def test_timeline_completeness_and_ordering(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
    client.post(f"/api/v1/cases/{cid}/acknowledge", json={}, headers=bkash_ops_headers)
    client.post(
...
def test_notification_list_and_read_persist(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
