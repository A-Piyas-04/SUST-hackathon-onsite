...
def _open_case(client, headers):
    run_id = start_run(client, headers, "scenario_b")
    published = publish(client, headers, run_id)
    alert = anomaly_alert(published)
    assert alert is not None, published
    resp = client.post(f"/api/v1/alerts/{alert['alert_id']}/cases", json={}, headers=headers)
    assert resp.status_code == 201, resp.text
    return resp.json()


def test_open_case_is_routed_and_idempotent(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
...
def test_legal_transition_matrix_enforced(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
...
def test_invalid_transition_rejected(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
...
def test_duplicate_acknowledge_is_illegal_transition(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
...
def test_resolution_precondition_requires_summary(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
    client.post(f"/api/v1/cases/{cid}/acknowledge", json={}, headers=bkash_ops_headers)
...
def test_optimistic_concurrency_conflict(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
...
def test_idempotent_duplicate_mutation(client, bkash_ops_headers):
    case = _open_case(client, bkash_ops_headers)
    cid = case["case_id"]
    key = f"ack-{cid}"
    first = client.post(
